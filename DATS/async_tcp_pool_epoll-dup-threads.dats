                ...
                let
                   // prval Some_v(pep) =  pep
                    ...
                    val () =
                      pool := (@{    
                          lfd = sfd
                        , efd = $UNSAFE.castvwtp1{epollfd}(0)//efd
            ...
            val _ = athread_create_cloptr_exn<>(
              llam() => 
                let 
                  ...
                  (** This may be useful to have as a function: dup the listening socket 

                      FIXME: A bit of a mess, cleanup
                  **)
                  val lfd2 = dup( socketfd_value( rpool.lfd ) )

                  val () = assertloc( lfd2 != ~1 )
                  val () = $UNSAFE.castvwtp0{void}( rpool.lfd )
                  val () = rpool.lfd := $UNSAFE.castvwtp1{socketfd1(listen)}( lfd2 )


                  val () = $UNSAFE.castvwtp0{void}( rpool.efd )
                  val () = rpool.efd := epollfd_create_exn()  

                  val () = assertloc( epollfd_add0( rpool.efd, rpool.lfd, EPOLLIN lor EPOLLET ) = 0 )
